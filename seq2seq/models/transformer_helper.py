...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        batchsize, num_head, tgt_time_step, embedd_dim = q.shape


        attn = self.dropout(F.softmax(attn, dim=-1))

        output = torch.matmul(attn, v).view(batchsize,1,tgt_time_step, num_head * embedd_dim ).squeeze(dim=1)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self,
        ...
        q = self.q_proj(query) / self.head_scaling
        k = self.k_proj(key) if key is not None else None
        v = self.v_proj(value) if value is not None else None


        q = q.contiguous().view( tgt_time_steps, batch_size * self.num_heads, self.head_embed_size).transpose(0,1)
        k = k.contiguous().view( -1, batch_size * self.num_heads, self.head_embed_size).transpose(0,1) if k is not None else None
        v = v.contiguous().view( -1, batch_size * self.num_heads, self.head_embed_size).transpose(0,1) if v is not None else None
        ...
        if key_padding_mask is not None:
            ...
            attn_weights = attn_weights.view(batch_size, self.num_heads, tgt_time_steps, src_time_steps)
        ...
        # attention =  ScaledDotProductAttention(temperature=self.head_scaling, attn_dropout=self.attention_dropout)
        #
        # if attn_mask is not None:
        #
        #     attn_mask = attn_mask.unsqueeze(0)   # For head axis broadcasting.
        #
        # attn, attn_weights = attention(q, k, v, mask=attn_mask)
        #
